...
import { getDashboardDataRequestAction, getDashboardDataSuccessAction, getDashboardDataFailureAction } from '../actions'
    ...
    try {
      await getDashboardDataAPI(params).then(response => {
        ...
        getDashboardDataSuccessAction({
          ...
          confirmedRevenue: numberWithSpaces(todayData.confirmed_reservations_data.revenue),
          canceledQuantity: todayData.canceled_reservations_data.quantity,
          canceledRevenue: numberWithSpaces(todayData.canceled_reservations_data.revenue),
        });
      });
    } catch (error) {
