...
class DatePicker extends PureComponent {
  ...
  render() {
    ...
    let nextDatePicker = helper.addMonth({
      month: this.props.month,
      year: this.props.year,
    });
    let prevDatePicker = helper.subtractMonth({
      month: this.props.month,
      year: this.props.year,
    });
    ...
    return (
      <Swiper
        next={c => this.props.next(c)}
        prev={c => this.props.prev(c)}
        swipeDuration={swipeDuration}>
        {
          <View style={{ width: '33.3333%' }}>
            {this.dates.getDates({
              ...params,
              month: prevDatePicker.month,
              year: prevDatePicker.year,
            })}
          </View>
        }
        {
          <View testID="couldBeTested" style={{ width: '33.3333%' }}>
            {this.dates.getDates(params)}
          </View>
        }
        {
          <View style={{ width: '33.3333%' }}>
            {this.dates.getDates({
              ...params,
              month: nextDatePicker.month,
              year: nextDatePicker.year,
            })}
          </View>
        }
      </Swiper>
